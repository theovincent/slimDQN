...
def save_data(p: dict, episode_returns: list, episode_lengths: list, model, rb, idx_epoch):
    ...
    os.makedirs(os.path.join(p["save_path"], f"replay_buffer/{p['seed']}"), exist_ok=True)
    ...
    rb.save(checkpoint_dir=rb_path, iteration_number=idx_epoch)
