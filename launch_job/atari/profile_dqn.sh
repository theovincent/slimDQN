...
if [[ $GPU = true ]]
then
    sbatch --job-name $EXPERIMENT_NAME-$ALGO_NAME --array=$FIRST_SEED-$LAST_SEED --cpus-per-task=3 --mem-per-cpu=$((N_PARALLEL_SEEDS * 6))G --time=02:00:00 --gres=gpu:1 --constraint="rtx2080" --partition stud --nodelist="cn02"\
