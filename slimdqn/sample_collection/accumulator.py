...
class Accumulator(checkpointers.Checkpointable, Protocol[ReplayElementT]):
    ...
    def accumulate(self, transition: elements.TransitionElement) -> Iterable[ReplayElementT]: ...
...
class TransitionAccumulator(Accumulator[elements.ReplayElement]):
    ...
    def __init__(
        ...
        self._trajectory = collections.deque[elements.TransitionElement](maxlen=self._update_horizon + self._stack_size)
    ...
    def accumulate(self, transition: elements.TransitionElement) -> Iterable[elements.ReplayElement]:
