...
class TestReplayBuffer(unittest.TestCase):
    ...
    def test_sample(self):
        ...
        self.assertEqual(
            len(self.rb.sample_transition_batch(batch_size)["observations"]), batch_size
        )
        for i in range(self.rb._replay_capacity):
            self.rb.add(
                observation=np.random.normal((2, 2)),
    ...
    def test_horizon_based_buffer(self):
        self.small_rb = ReplayBuffer(
            observation_shape=(2,), replay_capacity=6, batch_size=1, update_horizon=3
        )
        ...
        self.assertListEqual(
            self.small_rb.sample_transition_batch(1)["observations"][0].tolist(), [1, 1]
        )
        ...
        self.assertEqual(
            len(self.small_rb.sample_transition_batch(3)["observations"]), 3
        )
